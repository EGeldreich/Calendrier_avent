...
const updateH2 = () => {
    if (diffDays > 1) {
        // Change H2 depending on the days left before Xmas
        daysLeftEl.textContent = `${diffDays + 1} jours avant Noël !`;
    } else if (diffDays === 1) {
...
usedNumbers.forEach(
    (x) => boxEl[x - 1].classList.add(`day${usedNumbers[x - 1]}`) // Same logic as the <p> insertion for the -1
);
...
boxEl.forEach((box) => {
    ...
    box.addEventListener("click", () => {
        ...
        if (
            ...
            let storedDays =
                JSON.parse(localStorage.getItem("storedDays")) || []; // Get already stored days OR create empty array
